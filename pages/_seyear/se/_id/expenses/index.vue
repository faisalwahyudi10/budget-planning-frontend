      ...
      <div class="flex flex-wrap">
        <div class="w-full">
            <ul class="flex flex-row flex-wrap pt-3 pb-4 mb-0 list-none">
                ...
                <a class="block px-5 py-3 text-sm font-bold leading-normal uppercase rounded-lg shadow-lg" v-on:click="toggleTabs(1)" v-bind:class="{'text-primary bg-white dark:bg-slate-800': openTab !== 1, 'text-white bg-primary': openTab === 1}">
                    Perencanaan Rincian Belanja
                </a>
                </li>
                <li class="flex-auto mr-2 -mb-px text-center last:mr-0">
                <a class="block px-5 py-3 text-sm font-bold leading-normal uppercase rounded-lg shadow-lg" v-on:click="toggleTabs(2)" v-bind:class="{'text-gray-500 dark:text-gray-400 bg-white dark:bg-slate-800': openTab !== 2, 'text-white bg-gray-500': openTab === 2}">
                                    ...
                                    <table class="w-full mb-10 text-base text-left text-center text-gray-500 dark:text-gray-400">
                                        ...
                                        <tbody class="text-base text-gray-700 uppercase bg-gray-100 dark:bg-gray-700 dark:text-gray-400">
                                            ...
                                            <tr>
                                                ...
                                                <th scope="col" class="px-6 py-3" v-if="budgetTw1">
                                                    {{ budgetTw1 | currency('Rp. ') }}
                                                </th>
                                                <th scope="col" class="px-6 py-3" v-if="budgetTw2">
                                                    {{ budgetTw2 | currency('Rp. ') }}
                                                </th>
                                                <th scope="col" class="px-6 py-3" v-if="budgetTw3">
                                                    {{ budgetTw3 | currency('Rp. ') }}
                                                </th>
                                                <th scope="col" class="px-6 py-3" v-if="budgetTw4">
                                    ...
                                    <table class="w-full mt-5 text-sm text-left text-center text-gray-500 dark:text-gray-400">
                                        ...
                                        <tbody>
                                            <tr class="bg-white dark:bg-slate-800 border-b dark:bg-gray-800 dark:border-gray-700" v-for="(expense, index ) in resultQuery">
                                                ...
                                                <td class="px-4 py-4">{{ getMonthName(expense.month) + ' / Tw ' + expense.tw }}</td>
                        ...
                        <div v-bind:class="{'hidden': openTab !== 2, 'block': openTab === 2}">
                            <div class="pt-8 pb-11">
                                ...
                                <div class="relative overflow-x-auto">
                                    ...
                                    <table class="w-full mb-10 text-base text-left text-center text-gray-500 dark:text-gray-400">
                                        ...
                                        <tbody class="text-base text-gray-700 uppercase bg-gray-100 dark:bg-gray-700 dark:text-gray-400">
                                            ...
                                            <tr>
                                                ...
                                                <th scope="col" class="px-6 py-3" v-if="actual1">
                                                    {{ actual1 | currency('Rp. ') }}
                                                </th>
                                                <th scope="col" class="px-6 py-3" v-if="actual2">
                                                    {{ actual2 | currency('Rp. ') }}
                                                </th>
                                                <th scope="col" class="px-6 py-3" v-if="actual3">
                                                    {{ actual3 | currency('Rp. ') }}
                                                </th>
                                                <th scope="col" class="px-6 py-3" v-if="actual4">
                                                    {{ actual4 | currency('Rp. ') }}
                                                </th>
                                            </tr>
                                        </tbody>
                                    ...
                                    <div class="text-[22px] font-semibold text-dark dark:text-gray-100">Data Realisasi Rincian Belanja</div>
                                    ...
                                    <table class="w-full mt-5 text-sm text-left text-center text-gray-500 dark:text-gray-400">
                                        ...
                                        <tbody>
                                            <tr class="bg-white dark:bg-slate-800 border-b dark:bg-gray-800 dark:border-gray-700" v-for="(expense, index ) in resultQuery2">
                              ...
                              <div class="relative flex flex-col w-full bg-white dark:bg-slate-800 border-0 rounded-lg shadow-lg outline-none focus:outline-none">
                                ...
                                <div class="relative flex-auto p-6">
                                  <table class="shadow-lg shadow-blue-200/50 ... w-full rounded-lg border-separate border-spacing-2 border border-slate-400 ...">
                                    ...
                                    <tr>
                                      <th class="p-3 text-left">Bukti Nota</th>
                                      <td class="p-3"><input type="file" ref="file" name="image" class="w-100 bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" @change="getImage"/></td>
                                    </tr>
                                    <tr>
                                      <td colspan="2" class="p-3">
                                        <div class="flex flex-col items-center mb-[14px]">
                                            <img :src="file" v-if="file" class="object-fill w-full h-56 rounded-lg" alt="" />
                                            <div class="mt-6 mb-1 text-lg font-semibold">{{  }}</div>
...
export default {
    ...
    methods: {
      ...
      realizationData() {
        try {
            ...
            formData.append("amount_real", this.reData.amount_real);
            formData.append("realized", this.reData.realized);
            formData.append("confirmation_image", this.photo);
            this.$axios.post('/expense/realization/'+ this.reData.id, formData, config)
                .then(response => 
                    this.getExpenses() + this.getExpenses1() + this.getActivities() + this.getTotalTw() + this.getTotalRealTw()
                )
              this.getModal = false
    ...
    computed : {
        month() {
          if (this.getData.month == 1 || this.getData.month == 2 || this.getData.month == 3) {
            return this.getData.tw = 1;
          }
          if (this.getData.month == 4 || this.getData.month == 5 || this.getData.month == 6) {
            return this.getData.tw = 2;
          }
          if (this.getData.month == 7 || this.getData.month == 8 || this.getData.month == 9) {
            return this.getData.tw = 3;
          }
          if (this.getData.month == 10 || this.getData.month == 11 || this.getData.month == 12) {
            return this.getData.tw = 4;
          }
        },
        monthUpdate() {
          if (this.loadData.month == 1 || this.loadData.month == 2 || this.loadData.month == 3) {
            return this.loadData.tw = 1;
          }
          if (this.loadData.month == 4 || this.loadData.month == 5 || this.loadData.month == 6) {
            return this.loadData.tw = 2;
          }
          if (this.loadData.month == 7 || this.loadData.month == 8 || this.loadData.month == 9) {
            return this.loadData.tw = 3;
          }
          if (this.loadData.month == 10 || this.loadData.month == 11 || this.loadData.month == 12) {
            return this.loadData.tw = 4;
          }
        },
        years () {
